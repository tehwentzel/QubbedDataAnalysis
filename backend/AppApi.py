    ...
    for c,subdf in df.groupby('dose_clusters'):
        ...
        for organ in organ_list:
            ...
            for dcol in dose_cols:
                ...
                oentry[dcol] = qvals.values.astype(float).tolist()
            clust_entry[organ] = oentry
        ...
        for scol in s_cols:
            ...
            for (nweeks, pos) in date_positions:
                ...
                sentry[nweeks] = squants.values.astype(float).tolist()
            clust_entry[sname] = sentry
        ...
        for col in other_values:
            ...
            for val in unique:
                entry[str(val)] = {
                    'total': float((subdf[col] == val).sum()),
                    'mean': float((subdf[col] == val).mean()),
                }
            clust_entry[col] = entry
